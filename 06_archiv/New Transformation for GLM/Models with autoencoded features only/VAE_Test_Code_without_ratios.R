...
sourceFunctions <- function(functionDirectory = "/Users/admin/Desktop/Industry Lab/01_Code") {
  
  if (!dir.exists(functionDirectory)) {
    stop("Directory does not exist: ", functionDirectory)
  }
  ...
  message("Sourcing ", length(functionFiles), " R files...")
  
  invisible(lapply(functionFiles, source))
...
Data<-load("/Volumes/Data/WS2025/data.rda")
Data <- d  
...
for (col in num_cols) {
  res <- QuantileTransformation_new(Train[[col]], Test[[col]])
