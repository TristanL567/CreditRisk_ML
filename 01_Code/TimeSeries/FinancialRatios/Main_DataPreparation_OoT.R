...
#==== 03D - Data Sampling Out-of-Time =========================================#

## Function:
WalkForwardSplit <- function(Data, test_year) {
  dt <- as.data.table(Data)
  if (!"year" %in% names(dt)) {
    dt[, year := year(refdate)]
  }
  
  Train <- dt[year < test_year]
  Test <- dt[year == test_year]
...
tryCatch({
  ...
  message("--- Starting Out-of-Time (OOT) Data Split ---")
  
  holdout_year <- 2022 
  
  if(exists("WalkForwardSplit")) {
    Data_Sampled <- WalkForwardSplit(Data_Eng, test_year = holdout_year)
    Train <- Data_Sampled[["Train"]]
    Test  <- Data_Sampled[["Test"]]
    
  } else {
    warning("WalkForwardSplit not found. Fallback to basic time split.")
    
    # Fallback natively in data.table
    Data_Eng_dt <- as.data.table(Data_Eng)
    if(!"year" %in% names(Data_Eng_dt)) Data_Eng_dt[, year := year(refdate)]
    
    Train <- Data_Eng_dt[year < holdout_year]
    Test  <- Data_Eng_dt[year == holdout_year]
  }
  
  message(sprintf("OOT Split Complete. Train (< %d): %d rows | Test (== %d): %d rows", 
                  holdout_year, nrow(Train), holdout_year, nrow(Test)))
  
}, error = function(e) message("Error in Splitting: ", e))

### Generate the indices for CV.

cv_years <- c(2018:2021) 
Data_Train_CV_Split_IDs <- list()

for (y in cv_years) {
  fold_name <- paste0("Test_", y)
  Data_Train_CV_Split_IDs_Vector <- which(Train$year < y)
  test_indices <- which(Train$year == y)
  
  Data_Train_CV_Split_IDs[[fold_name]] <- test_indices
}

#==== 03E - OLD Data Sampling Out-of-Sample ===================================#
