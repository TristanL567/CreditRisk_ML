...
##==============================##
    ...
    tryCatch({
      ...
      bayes_out <- tryCatch({
        BayesianOptimization(
          ...
          kappa = 2.576,
          eps = 0.0,
          verbose = FALSE # Set to FALSE to keep console clean, your custom message handles it
        )
      }, error = function(e) {
      ...
      if(is.null(bayes_out)) stop("Bayesian Optimization returned NULL.")
      ...
      # Final CV to get exact optimal rounds
      ...
      stop(paste("Training Block Failed:", e$message))
    })
  ...
  # --- GENERAL PURPOSE FIX: Re-apply na.pass for the prediction matrix ---
  options(na.action = "na.pass")
  data_matrix  <- sparse.model.matrix(sparse_formula, data = Train_Data)
  options(na.action = old_na_action) # Restore defaults again
  # -----------------------------------------------------------------------
