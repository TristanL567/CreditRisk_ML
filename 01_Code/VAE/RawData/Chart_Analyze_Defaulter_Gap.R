...
Chart_Analyze_Gap <- function(data_subset, model, save_dir, file_suffix, title_suffix, analysis_target = "defaults") {
  ...
  tryCatch({
    ...
    Feature_Means <- Gap_Data %>%
      group_by(Group) %>%
      summarise(across(starts_with("f"), mean, .names = "{.col}")) %>%
    ...
    feature_map <- c(
      ...
      "f4"  = "Inventories", "f5"  = "Cash & Equiv.", "f6"  = "Equity",
      "f7"  = "Ret. Earnings", "f8"  = "Net Profit", "f9"  = "Profit C.F.",
