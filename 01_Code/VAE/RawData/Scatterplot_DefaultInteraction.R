...
Scatterplot_DefaultInteraction <- function(data_input, model, save_dir, file_name, x_feature = "f8", y_feature = "f6") {
  ...
  tryCatch({
    ...
    message(paste0("--- Starting Interaction Analysis: ", x_label, " vs. ", y_label, " ---"))
    ...
    # We select y, x_feature, and y_feature
    Forensic_DF <- data_input %>%
      ...
      # Filter: Keep ONLY Actual Defaults (Caught vs Missed)
      filter(Error_Type %in% c("False Negative (Missed Default)", "True Positive (Caught Default)"))
    
    # --- 4. Generate Scatter Plot ---
    p_scatter <- ggplot(Forensic_DF, aes(x = X_Value, y = Y_Value, color = Error_Type)) +
      ...
      # Remove threshold line (since axes are features, not probability)
    ...
    message(paste("Interaction plot saved to:", full_path))
    print(p_scatter)
