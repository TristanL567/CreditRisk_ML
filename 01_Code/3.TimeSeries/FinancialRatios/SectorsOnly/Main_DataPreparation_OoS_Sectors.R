...
tryCatch({
  ...
  ## Detect sector ratios: all numeric cols excluding meta/target/TS-derived
  ## (TS-derived features from 03A are excluded — sector stats on lagged
  ##  features would compound leakage risk and add noise)
  ...
  ## Size x sector ratios: those with enough group coverage in Train
  ## (at least MIN_SS_FIRMS firms per sector-size-year cell on average)
  MIN_SS_FIRMS <- if (exists("MIN_SS_FIRMS")) MIN_SS_FIRMS else 5L
  
  ss_coverage <- Train[, .(n = .N), by = .(sector, size, year)]
  size_sector_ratios <- if (mean(ss_coverage$n) >= MIN_SS_FIRMS) sector_ratios else {
    warning("  Low size-sector cell coverage — size x sector Z-scores will be sparse.")
    sector_ratios
  }
  ...
  ## Sector-level and global fallbacks (Train only)
  ...
  ## ── C. Join Stats → Apply Fallback Cascade ────────────────────────────────
  ...
  ## Fallback for Test rows with unseen sector-year
  ...
  ## ── F. Sector Percentile Rank (Train ECDF → Test) ─────────────────────────
  ...
  for (col in sector_ratios) {
    ...
    Test[is.na(get(out)), (out) := 0.5]   ## unseen sector-year → median rank
  }
  ...
  ## ── K. Sector Dummies ─────────────────────────────────────────────────────
  ## Derived from Train levels only — no hardcoded sector list
  ...
  dummy_levels  <- setdiff(sector_levels, ref_level)   ## drop most-frequent as reference
