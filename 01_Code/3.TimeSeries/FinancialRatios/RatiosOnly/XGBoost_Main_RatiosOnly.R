...
source("Subfunctions/XGBoost_Training_revised.R")
source("Subfunctions/XGBoost_Test_revised.R")
...
## Code.
##==============================##
XGBoost_Results_BaseModel <- XGBoost_Training_revised(
...
tryCatch({
  ...
  XGBoost_Test_Results_BaseModel <- XGBoost_Test_revised(Model = Model, 
                                                         Test_Data = Test_Data)
...
tryCatch({
  ...
  data_selected <- Train_Data_Strategy_C
  model_selected <- XGBoost_Results_Strategy_C$optimal_model
  save_directory <- file.path(Path, "03_Charts/VAE/FinancialRatios/XGBoost/StrategyC")
  
  Scatterplot_Interaction_Faceted_Ratios(
  ...
  Rich_Subset <- data_selected %>% filter(r8 > 0, r14 < 0)
