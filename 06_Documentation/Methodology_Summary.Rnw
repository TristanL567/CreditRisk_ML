...
## Chart Directories.
Directory_Charts <- file.path(Path, "03_Charts")
Directory_Charts_XGBoost <- file.path(Directory_Charts, "XGBoost")
...
\usepackage{geometry}
\usepackage{xcolor}
\usepackage[table]{xcolor} % <--- THIS IS THE FIX for \rowcolor

%% Charts.
\graphicspath{
    {C:/Users/TristanLeiter/Documents/Privat/ILAB/CreditRisk_ML/03_Charts/XGBoost/}
    {C:/Users/TristanLeiter/Documents/Privat/ILAB/CreditRisk_ML/03_Charts/GLM/}
    {C:/Users/TristanLeiter/Documents/Privat/ILAB/CreditRisk_ML/03_Charts/TestSet/}
}
...
Based on the instructions from the client, we filter the balance sheet data based on the following constraints to remove noisy observations. To account for floating-point inconsistencies, we add the condition that the difference must be greater than 2 (as per client instructions).
...
\begin{table}[h]
\centering
\caption{Financial positions (numeric) from the financial statement.}
\label{tab:financial_positions}
\begin{tabular}{lll}
\toprule
\textbf{Variable} & \textbf{Description (DE)} & \textbf{Description (EN)} \\ \midrule
...
f11 & Verbindlichkeiten & Liabilities \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Top 15 Risk Profiles (GLM Ranked). Despite the predominance of non-defaults ($0$), the model successfully concentrates defaults ($1$) in the top percentiles compared to the portfolio average.}
\label{tab:top_risk_actuals}
\begin{tabular}{ccccc}
\toprule
\textbf{Rank} & \textbf{Actual} & \textbf{GLM} & \textbf{Random Forest} & \textbf{XGBoost} \\
(Risk Order) & (Default) & ($P_{def}$) & ($P_{def}$) & ($P_{def}$) \\ \midrule
...
\rowcolor{gray!20} \textbf{8}  & \textbf{1} & \textbf{0.163} & \textbf{0.053} & \textbf{0.182} \\ 
...
\rowcolor{gray!20} \textbf{14} & \textbf{1} & \textbf{0.142} & \textbf{0.038} & \textbf{0.070} \\ 
15 & 0 & 0.140 & 0.027 & 0.156 \\ 
\bottomrule
\multicolumn{5}{l}{\footnotesize \textbf{Note}: Defaults (Rows 8, 14) are captured despite low absolute probabilities.} \\
\multicolumn{5}{l}{\footnotesize \textbf{Observation}: RF probabilities are consistently lower than GLM/XGB.} \\
\end{tabular}
\end{table}
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        % ADJUST PATH HERE
        \includegraphics[width=\textwidth]{01_GLM_HyperparameterTuningMethods_AUC_Training.png}
        \caption{Training Set AU for regularized GLMs.}
        \label{fig:GLM_train_AUC}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        % ADJUST PATH HERE
        \includegraphics[width=\textwidth]{05_GLM_CalibrationChart_BayesianOptimization_Test.png}
        \caption{Calibration Chart GLM(Test Set). Observed rates closely align with predictions, showing strong ranking ability.}
...
\begin{table}[h]
\centering
\caption{Hyperparameter Configuration and Model Performance (GLM). All optimization strategies converged to a Lasso-like solution ($\alpha \approx 1$).}
\label{tab:glm_model_comparison}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Mixing Param} & \textbf{Penalty Strength} & \textbf{CV AUC} \\
& ($\alpha$) & ($\lambda$) & (Validation) \\ \midrule
Grid Search & 1.00 & $3.03 \times 10^{-5}$ & 79.9\% \\ 
Random Search & 1.00 & $3.03 \times 10^{-5}$ & 79.9\% \\ 
\textbf{Bayesian Opt} & \textbf{1.00} & $\mathbf{3.04 \times 10^{-5}}$ & \textbf{79.9\%} \\ \bottomrule
\multicolumn{4}{l}{\footnotesize $\alpha$: Elastic Net mixing parameter ($0=$ Ridge, $1=$ Lasso). Controls variable selection;} \\
\multicolumn{4}{l}{\footnotesize $\lambda$: Regularization penalty strength. Controls the magnitude of coefficient shrinkage.} \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Summary of Final GLM Performance on the Test Set. The model is highly parsimonious, achieving 81.6\% AUC with just 19 variables (1-SE Rule).}
\label{tab:glm_final_results}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Best-fit} & \textbf{1-SE} \\
& (Lambda Min) & (1-SE Rule) \\ \midrule
\textbf{Test AUC} & \textbf{81.8\%} & 81.6\% \\
CV AUC & 79.9\% & -- \\
N Features & 20 & 19 \\ \midrule
\multicolumn{3}{l}{\textbf{Optimal Hyperparameters}} \\
Mixing Param ($\alpha$) & \multicolumn{2}{c}{1.00 (Lasso)} \\
Penalty ($\lambda$) & $3.04 \times 10^{-5}$ & $8.45 \times 10^{-4}$ \\ \bottomrule
\multicolumn{3}{l}{\footnotesize \textit{Note:} The 1-SE Rule removed one additional variable to improve parsimony with negligible performance loss.} \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Hyperparameter Configuration and Model Performance (Random Forest). The Bayesian optimizer improved generalization by selecting 'ExtraTrees' splits and specific node sizes.}
\label{tab:rf_model_comparison}
\begin{tabular}{lccccc}
\toprule
\textbf{Method} & \textbf{Vars/Split} & \textbf{Min Node} & \textbf{Sample Frac} & \textbf{Split Rule} & \textbf{CV AUC} \\
& (\texttt{mtry}) & (Size) & (Fraction) & (Type) & (Validation) \\ \midrule
Random Search & 4 & 37 & 0.39 & ExtraTrees & 78.9\% \\ 
\textbf{Bayesian Opt} & \textbf{3} & \textbf{48} & \textbf{0.87} & \textbf{ExtraTrees} & \textbf{79.1\%} \\ \bottomrule
\multicolumn{6}{l}{\footnotesize \texttt{mtry}: Number of variables candidates at each split;} \\
\multicolumn{6}{l}{\footnotesize Min Node: Minimum size of terminal nodes (controls depth/complexity);} \\
\multicolumn{6}{l}{\footnotesize ExtraTrees: Extremely Randomized Trees (randomly selected thresholds).} \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Summary of Final Random Forest Performance on the Test Set. The model achieves strong discrimination (81.0\% AUC) using sampling without replacement.}
\label{tab:rf_final_results}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Best-fit} \\
& (Bayesian Opt) \\ \midrule
\textbf{Test AUC} & \textbf{81.0\%} \\
CV AUC & 79.1\% \\
N Features & 21 \\ \midrule
\multicolumn{2}{l}{\textbf{Optimal Hyperparameters}} \\
Split Rule & ExtraTrees \\
Vars per Split (\texttt{mtry}) & 3 \\
Min Node Size & 48 \\
Sample Fraction & 0.87 (No Replace) \\ \bottomrule
\multicolumn{2}{l}{\footnotesize \textit{Note:} 'No Replace' indicates sampling without replacement, which can reduce bias.} \\
\end{tabular}
\end{table}
...
<<setup_dates, echo=FALSE>>=
# Start date for data context
...
\begin{table}[h]
\centering
\caption{Hyperparameter Configuration and Model Performance. Bayesian Optimization achieved the highest Mean Cross-Validation AUC.}
\label{tab:model_comparison}
\begin{tabular}{lccccc}
\toprule
\textbf{Method} & \textbf{Learn Rate} & \textbf{Tree Depth} & \textbf{Row Sample} & \textbf{Feat Sample} & \textbf{CV AUC} \\
& ($\eta$) & ($J$) & (Subsample) & (Colsample) & (Validation) \\ \midrule
Grid Search & 0.010 & 5 & 0.70 & 0.90 & 80.7\% \\ 
Random Search & 0.030 & 5 & 0.92 & 0.78 & 80.6\% \\ 
\textbf{Bayesian Opt} & \textbf{0.010} & \textbf{4} & \textbf{0.50} & \textbf{0.76} & \textbf{80.8\%} \\ \bottomrule
\multicolumn{6}{l}{\footnotesize \textit{Note:} CV AUC represents the mean performance across 5 stratified validation folds.} \\
\multicolumn{6}{l}{\footnotesize $\eta$: Learning Rate; $J$: Interaction Depth;} \\
\multicolumn{6}{l}{\footnotesize Subsample: Fraction of training instances (rows) used per tree;} \\
\multicolumn{6}{l}{\footnotesize Colsample: Fraction of features (columns) used per tree.} \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Summary of Final XGBoost Model Performance on the Test Set. The model utilizes the hyperparameters identified via Bayesian Optimization.}
\label{tab:xgboost_final_results}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Best-fit} & \textbf{1-SE} \\
& (Max AUC) & (1-SE Rule) \\ \midrule
\textbf{Test AUC} & \textbf{83.6\%} & 82.8\% \\
CV AUC & 80.8\% & -- \\
N Features & 21 & 21 \\ \midrule
\multicolumn{3}{l}{\textbf{Optimal Hyperparameters}} \\
Learning Rate ($\eta$) & \multicolumn{2}{c}{0.010} \\
Tree Depth ($J$) & \multicolumn{2}{c}{4} \\
Row Sampling & \multicolumn{2}{c}{0.50} \\
Feat. Sampling & \multicolumn{2}{c}{0.76} \\ \bottomrule
\multicolumn{3}{l}{\footnotesize \textit{Note:} The 1-SE model uses fewer boosting iterations to reduce overfitting variance.} \\
\end{tabular}
\end{table}
...
\begin{figure}[h]
    \centering
    % ADJUST PATH HERE
    \includegraphics[width=0.85\textwidth]{04_AUC_Test_Overall.png}
    \caption{Comparison of the Test-AUC of the models with the highest training AUC in each category.}
    \label{fig:test_auc}
\end{figure}

\begin{table}[h]
\centering
\caption{Final Model Comparison and Champion Selection. XGBoost is selected as the Champion Model as it outperforms the baseline (GLM) and Random Forest in both discrimination (AUC) and probability calibration (Brier Score).}
\label{tab:model_selection}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Test AUC} & \textbf{Brier Score} & \textbf{Rank} \\
&  &  & \\ \midrule
\textbf{XGBoost} & \textbf{83.6\%} & \textbf{0.00823} & \textbf{1} \\
Regularized GLM (1-SE) & 81.6\% & 0.00828 & 2 \\
Random Forest & 81.0\% & 0.00829 & 3 \\
Neural Network & 80.2\% & 0.00834 & 4 \\ \bottomrule
\multicolumn{4}{l}{\footnotesize \textit{Note:} While AUC measures ranking, the lower Brier Score of XGBoost confirms superior} \\
\multicolumn{4}{l}{\footnotesize risk quantification, particularly in the high-risk tail.} \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Summary of Final XGBoost Model Performance on the Test Set. The model achieves the highest discrimination and calibration accuracy of all candidates.}
\label{tab:xgboost_final_results}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Best-fit} & \textbf{1-SE} \\
& (Max AUC) & (1-SE Rule) \\ \midrule
\textbf{Test AUC} & \textbf{83.6\%} & 82.8\% \\
\textbf{Brier Score} & \textbf{0.0082} & -- \\
CV AUC & 80.8\% & -- \\
N Features & 21 & 21 \\ \midrule
\multicolumn{3}{l}{\textbf{Optimal Hyperparameters}} \\
Learning Rate ($\eta$) & \multicolumn{2}{c}{0.010} \\
...
%==== Chapter 6: Model Improvements ===========================================%

\chapter{Model Improvements}

\section{Feature Engineering via Autoencoders}

To capture non-linear structural anomalies in the balance sheets that tree-based models might miss, we augmented the feature space using an \textbf{Autoencoder (AE)}. An Autoencoder is an unsupervised neural network designed to learn a compressed representation (encoding) of the input data and subsequently reconstruct it.

\subsection*{Concept: Reconstruction Error as Anomaly Score}
The Autoencoder is trained exclusively on ``healthy'' (non-defaulting) firms to learn the manifold of normal financial positions. When the model encounters a distressed firm, it fails to reconstruct the financial ratios accurately, resulting in a high \textbf{Reconstruction Error}. This error serves as a powerful proxy for financial distress (Anomaly Score). Additionally, we extract the \textbf{Latent Features} from the bottleneck layer, which represent non-linear combinations of the original variables.

\subsection*{Implementation: Out-of-Fold (OOF) Stacking}
Integrating unsupervised features into a supervised pipeline requires strict prevention of data leakage. We implemented a \textbf{Two-Stage Out-of-Fold Strategy} prior to the XGBoost hyperparameter tuning:

\begin{enumerate}
    \item \textbf{OOF Feature Generation:} We split the training data into $k=5$ stratified folds. For each fold, an Autoencoder is trained on the remaining $k-1$ folds and generates anomaly scores for the hold-out fold. This ensures that the features for any given firm are generated by a model that has never seen that firm, preventing overfitting.
    \item \textbf{Feature Space Freezing:} The OOF predictions are stacked to create a fully augmented training set. A final Autoencoder is trained on the entire training set to generate features for the test set.
    \item \textbf{Bayesian Optimization Integration:} These augmented features (Reconstruction Error + Latent Vectors) are treated as static inputs during the XGBoost Bayesian Optimization. By ``freezing'' the feature space beforehand, we avoid the computational cost of retraining the neural network inside every iteration of the Bayesian search, allowing for an efficient and extensive search of the XGBoost hyperparameter space.
\end{enumerate}
