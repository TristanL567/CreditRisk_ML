...
% Path for charts.
\graphicspath{
    {C:/Users/Tristan Leiter/Documents/ILAB_OeNB//CreditRisk_ML/03_Charts/VAE/XGBoost/BaseModel}
    {C:/Users/Tristan Leiter/Documents/ILAB_OeNB//CreditRisk_ML/03_Charts/VAE/XGBoost/StrategyA}
}
...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        \textbf{Model} & \textbf{AUC} & \textbf{Brier Score (\%)} & \textbf{Pen. Brier Score (\%)} \\
        \hline
        Strategy C (Feature Denoising) & 0.8302 & 0.8031\% & 1.2287\% \\
        Strategy B (Anomaly Score) & 0.8291 & 0.7869\% & 1.2125\% \\
        Base Model & 0.8279 & 0.8031\% & 1.2287\% \\
        Strategy A (Dim. Reduction) & 0.8276 & 0.8041\% & 1.2297\% \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Strategy} & \textbf{Eta ($\eta$)} & \textbf{Max Depth} & \textbf{Subsample} & \textbf{Colsample} \\
        \hline
        Base Model & 0.01 & 5 & 0.619 & 1.000 \\
        Strategy A (Dim. Reduction) & 0.01 & 5 & 0.685 & 0.500 \\
        Strategy B (Anomaly Score) & 0.01 & 6 & 0.860 & 0.838 \\
        Strategy C (Feature Denoising) & 0.01 & 5 & 0.594 & 0.877 \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[h]
    \centering
    \caption{Forensic Feature Summary: Strategy [Name]} % Update Strategy Name as needed
    \label{tab:forensic_summary_new}
    \resizebox{\textwidth}{!}{% Resize table to fit page width if necessary
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Error Type} & \textbf{Count} & \textbf{RoA} & \textbf{Net Debt Ratio} & \textbf{Self-Financing Ratio} & \textbf{Return Fixed Assets} & \textbf{Cash to Current Assets} \\
        \midrule
        True Negative  & 125,115 & 0.2043 & -0.2681 & 0.2577 & 0.1566 & 0.2197 \\
        False Positive & 34,915  & -0.9443 & 0.9210 & -1.0014 & -0.9687 & -0.6707 \\
        False Negative & 208     & -0.0856 & 0.0819 & -0.0080 & -0.1060 & -0.0034 \\
        True Positive  & 1,166   & -1.1825 & 1.0523 & -1.1682 & -1.1846 & -0.6429 \\
        \bottomrule
        \multicolumn{7}{l}{\footnotesize \textit{Note: All financial feature columns reflect the median values for that group.}} \\
    \end{tabular}%
    }
...
\begin{table}[h]
    \centering
    \caption{Forensic Feature Summary: Strategy [Name]}
    \label{tab:forensic_summary_strat_updated_3}
    \resizebox{\textwidth}{!}{% Resize table to fit page width if necessary
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Error Type} & \textbf{Count} & \textbf{RoA} & \textbf{Net Debt Ratio} & \textbf{Self-Financing Ratio} & \textbf{Return Fixed Assets} & \textbf{Cash to Current Assets} \\
        \midrule
        True Negative  & 129,267 & 0.1815 & -0.2355 & 0.2258 & 0.1430 & 0.1904 \\
        False Positive & 30,763  & -0.9860 & 0.9710 & -1.0549 & -1.0131 & -0.6643 \\
        False Negative & 153     & -0.0718 & 0.1221 & 0.0031 & -0.0457 & 0.0265 \\
        True Positive  & 1,221   & -1.1454 & 1.0019 & -1.1390 & -1.1341 & -0.6204 \\
        \bottomrule
        \multicolumn{7}{l}{\footnotesize \textit{Note: All financial feature columns reflect the median values for that group.}} \\
    \end{tabular}%
    }
...
\begin{table}[h]
    \centering
    \caption{Forensic Feature Summary: Strategy [Name]}
    \label{tab:forensic_summary_updated}
    \resizebox{\textwidth}{!}{% Resize table to fit page width if necessary
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Error Type} & \textbf{Count} & \textbf{RoA} & \textbf{Net Debt Ratio} & \textbf{Self-Financing Ratio} & \textbf{Return Fixed Assets} & \textbf{Cash to Current Assets} \\
        \midrule
        True Negative  & 125,303 & 0.2042 & -0.2674 & 0.2570 & 0.1594 & 0.2161 \\
        False Positive & 34,727  & -0.9529 & 0.9217 & -1.0125 & -0.9753 & -0.6646 \\
        False Negative & 232     & -0.0523 & 0.0819 & -0.0165 & -0.0292 & -0.0660 \\
        True Positive  & 1,142   & -1.2016 & 1.0798 & -1.1781 & -1.2037 & -0.6395 \\
        \bottomrule
        \multicolumn{7}{l}{\footnotesize \textit{Note: All financial feature columns reflect the median values for that group.}} \\
    \end{tabular}%
    }
...
\section{Financial Ratios (Feature Eng.)}

\subsection{Training Results}

The leaderboard below summarizes the predictive accuracy of each strategy. Strategy B (Anomaly Score) achieves the highest discrimination, while Strategy A (Dimensionality Reduction) provides the most accurately calibrated risk probabilities[cite: 1, 2].
...
\begin{tabular}{lcccc}
\hline
\textbf{Model Strategy} & \textbf{AUC} & \textbf{Uplift (\%)} & \textbf{Brier Score} & \textbf{Pen. Brier} \\ 
\hline
Strategy B (Anomaly Score)      & 0.8304 & +0.116\% & 0.7917\% & 1.2173\% \\
Strategy A (Dim. Reduction)     & 0.8295 & +0.016\% & 0.7790\% & 1.2046\% \\
Base Model                      & 0.8294 & ---      & 0.7999\% & 1.2255\% \\
Strategy C (Feature Denoising)  & 0.8289 & -0.059\% & 0.8035\% & 1.2292\% \\
\hline
\end{tabular}
\end{table}
...
To ensure a fair comparison across different feature spaces, we employed Bayesian Optimization to identify the specific hyperparameter configurations that minimize validation loss for each strategy[cite: 3, 5].



\begin{table}[H]
\centering
\caption{Optimal XGBoost Configurations per Strategy}
\label{tab:params}
\begin{tabular}{lccccc}
\hline
\textbf{Strategy} & \textbf{Eta ($\eta$)} & \textbf{Depth} & \textbf{Subsample} & \textbf{Colsample} & \textbf{Rounds} \\ 
\hline
Base Model                     & 0.0100 & 4 & 0.746 & 0.500 & 804 \\
Strategy A (Dim. Reduction)    & 0.0100 & 6 & 0.683 & 0.603 & 428 \\
Strategy B (Anomaly Score)     & 0.0100 & 5 & 0.500 & 0.500 & 548 \\
Strategy C (Feature Denoising) & 0.0132 & 4 & 0.816 & 0.805 & 470 \\
\hline
\end{tabular}
\end{table}

\subsection{Key Forensic Takeaways}

\begin{itemize}
    \item \textbf{Efficiency Gain:} While the Base Model remains highly competitive, it requires significantly more boosting rounds (804) to reach optimal convergence compared to the engineered strategies.
    \item \textbf{Discriminatory Power:} Strategy B proves that explicitly modeling "weirdness" via Anomaly Scores provides the best ranking of high-risk defaults[cite: 1].
    \item \textbf{Calibration:} Strategy A excels in calibration, as evidenced by the lowest Brier Score, making it the preferred choice for applications requiring precise probability estimates rather than just binary classification[cite: 2].
\end{itemize}
...
\section{Performance of Financial-Ratios}
...
\section{Raw Balance Sheet Items}
...
\subsection{Standard Ratios}

\begin{table}[h]
    \centering
    \caption{Model Performance Leaderboard (Final Results)}
    \label{tab:leaderboard_final}
    \begin{tabular}{lccc}
        \hline
        \textbf{Model} & \textbf{AUC} & \textbf{Brier Score (\%)} & \textbf{Pen. Brier Score (\%)} \\
        \hline
        \textbf{Base Model (Winner)} & \textbf{0.8321} & 0.8053\% & 1.2212\% \\
        Strategy B (Anomaly Score) & 0.8318 & 0.8034\% & 1.2193\% \\
        Strategy C (Feature Denoising) & 0.8318 & \textbf{0.8033\%} & \textbf{1.2192\%} \\
        Strategy A (Dim. Reduction) & 0.8304 & 0.8051\% & 1.2210\% \\
        \hline
    \end{tabular}
\end{table}

\subsection{Feature engineered Ratios}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model Strategy} & \textbf{AUC} & \textbf{Brier Score (\%)} & \textbf{Pen. Brier (\%)} & \textbf{Log Loss} \\ 
\midrule
Base Model \textbf{(Winner)}    & 0.8306 & 0.8055\% & 1.2214\% & 0.0413 \\
Strategy A (Dim. Reduction)     & 0.8303 & 0.8054\% & 1.2213\% & 0.0413 \\
Strategy B (Anomaly Score)      & 0.8301 & 0.8054\% & 1.2213\% & 0.0413 \\
Strategy C (Feature Denoising)  & 0.8289 & 0.8059\% & 1.2218\% & 0.0413 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
    \centering
    \caption{Final Model Performance Leaderboard (Test Set)}
    \label{tab:final_leaderboard}
    \begin{tabular}{llccc}
        \hline
        \textbf{Model} & \textbf{Strategy} & \textbf{Test-AUC} & \textbf{Brier Score (\%)} & \textbf{Pen. Brier Score (\%)} \\
        \hline
        \textbf{XGBoost Ratios} & \textbf{Strategy C} & \textbf{83.18\%} & \textbf{0.80\%} & \textbf{1.22\%} \\
        XGBoost & Base Model & 82.05\% & 0.83\% & 1.25\% \\
        GLM & Strategy A & 81.98\% & 0.83\% & 1.27\% \\
        GLM Ratios & Strategy A & 81.69\% & 0.84\% & 1.26\% \\
        Random Forest & Base Model & 81.32\% & 0.83\% & - \\
        \hline
    \end{tabular}
\end{table}
