...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        \textbf{Model} & \textbf{AUC} & \textbf{Brier Score (\%)} & \textbf{Pen. Brier Score (\%)} \\
        \hline
        Strategy C (Feature Denoising) & 0.8278 & 0.8100\% & 1.2357\% \\
        Strategy B (Anomaly Score) & 0.8268 & 0.7663\% & 1.1919\% \\
        Base Model & 0.8264 & 0.8136\% & 1.2392\% \\
        Strategy A (Dim. Reduction) & 0.8247 & 0.7490\% & 1.1746\% \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Strategy} & \textbf{Eta ($\eta$)} & \textbf{Max Depth} & \textbf{Subsample} & \textbf{Colsample} \\
        \hline
        Base Model & 0.01 & 3 & 0.504 & 1.000 \\
        Strategy A (Dim. Reduction) & 0.01 & 8 & 0.843 & 0.500 \\
        Strategy B (Anomaly Score) & 0.01 & 8 & 0.500 & 1.000 \\
        Strategy C (Feature Denoising) & 0.01 & 4 & 0.500 & 0.996 \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[h]
    \centering
    \caption{Final Model Performance Leaderboard}
    \label{tab:final_leaderboard}
    \begin{tabular}{lccccc}
        \hline
        \textbf{Model} & \textbf{AUC} & \textbf{Brier (\%)} & \textbf{Pen. Brier (\%)} & \textbf{Log Loss} & \textbf{Time (s)} \\
        \hline
        Base Model & 0.8310 & 0.8058\% & 1.2217\% & 0.0413 & 0.19 \\
        Strategy B (Anomaly Score) & 0.8263 & 0.8061\% & 1.2220\% & 0.0415 & 0.15 \\
        Strategy C (Feature Denoising) & 0.8234 & 0.8080\% & 1.2239\% & 0.0417 & 0.13 \\
        Strategy A (Dim. Reduction) & 0.8177 & 0.8099\% & 1.2258\% & 0.0422 & 0.14 \\
        \hline
    \end{tabular}
\end{table}
\subsection{Boosting}
