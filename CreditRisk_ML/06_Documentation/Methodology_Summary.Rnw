...
\begin{itemize}
    \item \textbf{Data Preprocessing:} \texttt{DataPreprocessing.R} applies balance sheet consistency filters (Section 1.1.1)
    \item \textbf{Feature Engineering:} \texttt{QuantileTransformation.R} implements the Probability Integral Transform (Section 2.2)
    \item \textbf{Stratified Sampling:} \texttt{MVstratifiedsampling.R} ensures firm-level data partitioning (Section 1.1.2)
...
## Source the main analysis script (lines 1-173 for setup and data prep)
## Then execute lines 195-394 for GLM model training and evaluation
...
\subsubsection{Hyperparameter Tuning Comparison}
...
<<glm_load_results, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE>>=
## Load GLM results from Main.R execution
source(file.path(Directory, "Load_GLM_Results.R"), local = TRUE)
@

<<glm_results_table, echo=FALSE, eval=TRUE, results='asis'>>=
library(xtable)
print(xtable(glm_results_table, 
             caption = "GLM Hyperparameter Tuning Method Comparison. Results obtained from Main.R execution (Section 04 - GLMs).",
...
<<glm_champion_table, echo=FALSE, eval=TRUE, results='asis'>>=
if(exists("glm_champion_summary")) {
  print(xtable(glm_champion_summary, 
               caption = "Champion GLM Model Summary (Bayesian Optimization)",
               label = "tab:glm_champion",
               digits = 4,
               align = c("l", "l", "l")),
        include.rownames = FALSE,
        caption.placement = "top",
...
\textit{Note: To populate these tables with actual results, execute the following commands in R:}
...
\textit{This will run the complete GLM workflow from Main.R and export results to the Documentation/Results directory.}
...
## Visualizations generated by Main.R (lines 366-394):
...
Following the methodology outlined in Section 2.1, the champion GLM model is selected based on the highest cross-validation AUC across all hyperparameter tuning methods. The 1-standard-error (1-SE) rule is also evaluated to assess the trade-off between model complexity and generalization.
